...
const BugListPage = () => {
  ...
  const fetchBugs = useCallback(async () => {
    try {
      ...
      const apiCall =
        user?.role === "CUSTOMER" ? bugsAPI.getMy : bugsAPI.getAll;
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {bugs.map((bug) => {
                  const pConfig =
                    priorityConfig[bug.priority] || priorityConfig.MEDIUM;
                  ...
                  return (
                    ...
                    >
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              BUG-{String(bugId).slice(-6).toUpperCase()}
                            </p>
                          </div>
                        </Link>
                      </td>
                      <td className="py-3 px-4">
                        <span
                          className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${pConfig.bg} ${pConfig.text}`}
                        >
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${pConfig.dot}`}
                          />
                          {bug.priority}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${sConfig.bg} ${sConfig.text}`}
                        >
