...
export const login = async (req, res, next) => {
  try {
    ...
    const user = await User.findOne({ email: email.toLowerCase().trim() })
      .select("+passwordResetToken +passwordResetExpires +refreshToken +loginAttempts +lockUntil");
...
export const forgotPassword = async (req, res, next) => {
  try {
    ...
    // In production, send email here. For now expose token in non-prod environments.
